...
- [x] T028 [P] [US2] Create `src/components/WineCard.astro` (optimised `<Image/>` thumbnail or placeholder, nombre, bodega, anada; `data-*` for filtering; future-vintage marker via `vintage.ts`) (FR-004, FR-013).
- [x] T029 [P] [US2] Create `src/components/EmptyState.astro` (empty + no-results states) (FR-016).
- [x] T030 [US2] Create `src/components/WineGrid.astro` (queries the `vinos` collection, renders the card grid) (FR-004).
- [x] T031 [US2] Create `src/pages/index.astro` (BaseLayout + WineGrid + empty state) — makes T025/T027 pass (FR-004, FR-016).
- [x] T032 [US2] Create `src/pages/vinos/[slug].astro` with `getStaticPaths()` over `vinos`: full `<Image/>`, all fields, rendered `notas` — makes T026 pass (FR-005, FR-020, FR-022).
