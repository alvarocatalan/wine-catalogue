...
- **Implementation note / risk to verify first**: Keystatic's image field
  `directory` (`src/assets/vinos`) and **`publicPath`** must be set so the stored
  frontmatter path is resolvable by the `image()` helper (relative to the entry).
  This Keystatic↔`astro:assets` seam is the first thing to validate in
  implementation: author one wine → `astro build` → assert an optimised `<img>`
  with a hashed `src` and the correct `alt`.
